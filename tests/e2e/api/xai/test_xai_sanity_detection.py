...
class TestOVDetXAIAPI:
    ref_raw_saliency_shapes = {
        "MobileNetV2-ATSS": (16, 16),  # Need to be adapted to configurable or adaptive input size
    }

    def test_inference_xai(self, tmp_dir_path):
        hyper_parameters, model_template = DetectionTaskAPIBase.setup_configurable_parameters(
        ...
        task_env, dataset = DetectionTaskAPIBase.init_environment(hyper_parameters, model_template, 10)
        ...
        save_model_data(trained_model, tmp_dir_path)

        del task_env, dataset, train_task
        ...
        for processed_saliency_maps, only_predicted in [[True, False], [False, True]]:
            ...
            # Save OV IR model
            inference_task._model_ckpt = osp.join(tmp_dir_path, "weights.pth")
            ...
            del task_env.model, exported_model, inference_task
            ...
            del dataset
            _, dataset = DetectionTaskAPIBase.init_environment(hyper_parameters, model_template, 10)
