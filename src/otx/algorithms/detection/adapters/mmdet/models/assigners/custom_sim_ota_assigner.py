"""Custom assigner to workaround numerical issues when using IPEX."""
...
class CustomSimOTAAssigner(SimOTAAssigner):
    ...
    This assigner adds clamp before F.binary_cross_entropy to avoid failures
    ...
    def __init__(self, center_radius=2.5, candidate_topk=10, iou_weight=3.0, cls_weight=1.0):
        super().__init__(center_radius, candidate_topk, iou_weight, cls_weight)
    ...
    def _assign(self, pred_scores, priors, decoded_bboxes, gt_bboxes, gt_labels, gt_bboxes_ignore=None, eps=1e-7):
        ...
        cls_cost = (
            F.binary_cross_entropy(
                ...
                .sqrt_()
                .clamp_(0, 1),  # clamp is required for numerical stability
                gt_onehot_label,
                reduction="none",
            )
            .sum(-1)
